...
for(eval_dir in evaluations) {
  ...
  kd.table <- read.csv(paste0(results_dir,"/PositionWiseKdEstimates",suffix,".csv"), header=T, sep="\t")
  kd.table_all <- rbind(kd.table_all, data.frame(kd.table, comparison=eval_dir))
  
  max_kd.table <- read.csv(paste0(results_dir,"/PositionWiseMaxKd",suffix,".csv"), header=T, sep="\t")
